...
The fundamental problem of causal inference is that it is impossible to observe the true individual treatment effect and therefore it is challenging to determine the causal effect of a treatment. This issue stems from the fact that it's impossible to observe both potential outcomes of treatment and control; if the unit receives the treatment it is impossible to observe the individual effect of the counterfactual (receiving no treatment) and vice versa.

2. Why are experiments important? (5 points)

Experiments, especially randomized controlled trials (RCTs) are the fundamental "solution" to the fundamental problem of causal inference. Experiments are studies where the researcher controls the probability of being assigned the treatment. One can engineer the experiment so that the requirements for average treatment effect (ATE) are met. The three requirements are SUTVA, positivity, and ignorability. SUTVA
...
print(mean(STAR$g4math[STAR$kinder == "small"], na.rm = T))
print(mean(STAR$g4math[STAR$kinder == "regular"], na.rm = T))
print(mean(STAR$g4math[STAR$kinder == "regular with aid"], na.rm = T))

print(mean(STAR$g4reading[STAR$kinder == "small"], na.rm = T))
print(mean(STAR$g4reading[STAR$kinder == "regular"], na.rm = T))
print(mean(STAR$g4reading[STAR$kinder == "regular with aid"], na.rm = T))
